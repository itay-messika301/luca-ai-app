...
const DOC_STATUS_LABELS = {
  pending:      { label: 'ממתין לעיבוד', color: 'text-white/40' },
    ...
    <div className="p-6 max-w-4xl" dir="rtl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-violet-500/20 flex items-center justify-center text-white text-lg font-bold">
            {(client.business_name || '?').charAt(0).toUpperCase()}
          </div>
          <div>
            <h1 className="text-white text-xl font-bold">{client.business_name}</h1>
            ...
            {editing ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/60 rounded-lg text-xs transition-colors"
                >
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white rounded-lg text-xs transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/60 hover:text-amber-400 rounded-lg text-xs transition-colors"
                >
                  {client.archived_at
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="md:col-span-2 bg-white/5 border border-white/10 rounded-xl p-5">
          <h2 className="text-white font-semibold mb-4 text-sm">פרטי הלקוח</h2>
              ...
              {editing ? (
                <div>
                  <input
                    ...
                    className={`w-full bg-white/5 border rounded-lg px-2.5 py-1.5 text-white text-sm font-mono focus:outline-none ${regError ? 'border-red-500' : 'border-white/15 focus:border-blue-500'}`}
                  />
            ...
            <Field label="שם העסק" editing={editing}>
              {editing ? (
                <input
                  ...
                  className="w-full bg-white/5 border border-white/15 rounded-lg px-2.5 py-1.5 text-white text-sm focus:outline-none focus:border-blue-500"
                />
              ...
              {editing ? (
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none bg-white/5 border border-white/15 rounded-lg px-2.5 pl-7 py-1.5 text-white text-sm focus:outline-none cursor-pointer"
                  >
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
          <h2 className="text-white font-semibold text-sm">סטטיסטיקות</h2>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-5">
        <h2 className="text-white font-semibold mb-4 text-sm">מסמכים אחרונים</h2>
        {documents.length === 0 ? (
          <div className="text-center py-8">
            <FileText className="w-8 h-8 text-white/15 mx-auto mb-2" />
            <p className="text-white/30 text-sm">אין מסמכים עדיין</p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {documents.map(doc => {
              const statusInfo = DOC_STATUS_LABELS[doc.review_status] || DOC_STATUS_LABELS[doc.status] || { label: doc.review_status || doc.status, color: 'text-white/40' }
              return (
                <div key={doc.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-white/3 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-2.5">
                    <FileText className="w-4 h-4 text-white/30 flex-shrink-0" />
                    <span className="text-white/70 text-sm truncate max-w-xs">{doc.file_name || 'מסמך'}</span>
                  </div>
                  <div className="flex items-center gap-4 flex-shrink-0">
                    {doc.total_amount && (
                      <span className="text-white/40 text-xs">₪{Number(doc.total_amount).toLocaleString('he-IL')}</span>
                    )}
                    <span className={`text-xs ${statusInfo.color}`}>{statusInfo.label}</span>
                    <span className="text-white/20 text-xs">{new Date(doc.created_at).toLocaleDateString('he-IL')}</span>
                  </div>
                </div>
              )
            })}
...
function Field({ label, editing, children }) {
  return (
    <div>
      ...
      <div className={`text-sm ${editing ? '' : 'text-white/80'}`}>{children}</div>
    </div>
  )
}

function Stat({ label, value, color = 'text-white/80' }) {
  return (
