...
export default function Dashboard() {
  ...
  async function fetchStats() {
    const [clientsRes, docsRes, recentRes] = await Promise.all([
      ...
      supabase.from('documents').select('id, file_name, status, created_at, clients(company_name)').order('created_at', { ascending: false }).limit(5)
    ])
    ...
    <div className="p-6 max-w-5xl mx-auto" dir="rtl">
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-400 text-sm">לא הועלו מסמכים עדיין</div>
        ) : (
              ...
              <div key={doc.id} className={`flex items-center justify-between px-5 py-3 ${i !== 0 ? 'border-t border-gray-50' : ''}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{doc.clients?.company_name}</p>
                  </div>
                </div>
