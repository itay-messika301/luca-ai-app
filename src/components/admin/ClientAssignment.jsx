...
export default function ClientAssignment() {
  ...
  useEffect(() => {
    ...
    ]).then(([clientsRes, accountantsRes]) => {
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          {unassignedCount > 0 && (
            <p className="text-sm text-orange-600 mt-0.5">{unassignedCount} לקוחות ללא שיוך</p>
          )}
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600 text-right">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">משויך ל</th>
              <th className="px-4 py-3 font-medium">שינוי שיוך</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filtered.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">לא נמצאו לקוחות</td></tr>
            )}
            {filtered.map(client => (
              <tr key={client.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900">{client.name}</td>
                <td className="px-4 py-3 text-gray-500 font-mono text-xs">{client.vat_number || '—'}</td>
                <td className="px-4 py-3">
                  {client.assigned_accountant_id ? (
                    ...
                    <span className="inline-flex items-center px-2 py-0.5 bg-orange-100 text-orange-700 rounded-full text-xs font-medium">לא משויך</span>
                  )}
                </td>
                <td className="px-4 py-3">
                    ...
                    >
                      ...
                      {accountants.map(acc => (
                        <option key={acc.id} value={acc.id}>
                          {acc.full_name || acc.email}{acc.role === 'office_manager' ? ' (מנהל)' : ''}
                        </option>
                      ))}
                    </select>
                    {saving === client.id && <span className="text-xs text-blue-500 animate-pulse">שומר...</span>}
                  </div>
                </td>
              </tr>
            ))}
