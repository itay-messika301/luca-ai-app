...
export default function CSVImporter({ existingNumbers = new Set(), onImport, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" dir="rtl">
      <div className="bg-[#111117] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-white/10 flex-shrink-0">
          <h2 className="text-white font-bold text-lg">ייבוא לקוחות מ-CSV</h2>
          ...
          {!file ? (
            <>
              ...
              >
                ...
                <p className="text-white/60 text-sm">גרור קובץ CSV לכאן, או לחץ לבחירה</p>
                <p className="text-white/30 text-xs mt-1">תמיכה בקובץ CSV עד 5MB</p>
                <input
                  ref={inputRef}
                  type="file"
                  accept=".csv"
              ...
              <div className="bg-white/5 rounded-xl p-4 text-xs text-white/40 space-y-1">
                <p className="font-medium text-white/60 mb-2">עמודות נדרשות:</p>
                <p><span className="text-white/70">registration_number</span> — מספר ח.פ (9 ספרות)</p>
                <p><span className="text-white/70">business_name</span> — שם העסק</p>
                <p><span className="text-white/50">owner_name</span> — שם הבעלים (אופציונלי)</p>
                <p><span className="text-white/50">reporting_cycle</span> — monthly / bimonthly (אופציונלי)</p>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="flex items-center gap-3 bg-white/5 rounded-xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/40 text-xs">{preview?.total} שורות נמצאו</p>
                </div>
                <button
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-center">
                      ...
                      <p className="text-white/50 text-xs">תקינות</p>
                    </div>
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3 text-center">
                      ...
                      <p className="text-white/50 text-xs">עם אזהרות</p>
                    </div>
                    <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-center">
                      ...
                      <p className="text-white/50 text-xs">שגויות (מדולגות)</p>
                    </div>
                  </div>
                    ...
                    <div className="border border-white/10 rounded-xl overflow-hidden">
                      <table className="w-full text-xs">
                        <thead className="bg-white/5">
                          <tr>
                            <th className="text-right text-white/50 px-3 py-2 font-medium">מספר ח.פ</th>
                            <th className="text-right text-white/50 px-3 py-2 font-medium">שם העסק</th>
                            <th className="text-right text-white/50 px-3 py-2 font-medium">בעלים</th>
                            <th className="text-right text-white/50 px-3 py-2 font-medium">דיווח</th>
                          </tr>
                        </thead>
                        <tbody>
                          {preview.valid.slice(0, 5).map((r, i) => (
                            <tr key={i} className="border-t border-white/5">
                              <td className="px-3 py-2 text-white/70 font-mono">{r.data.registration_number}</td>
                              <td className="px-3 py-2 text-white/80">{r.data.business_name}</td>
                              <td className="px-3 py-2 text-white/50">{r.data.owner_name || '—'}</td>
                              <td className="px-3 py-2 text-white/50">{r.data.reporting_cycle === 'monthly' ? 'חודשי' : 'דו-חודשי'}</td>
                            </tr>
                          ))}
                          {preview.valid.length > 5 && (
                            <tr className="border-t border-white/5">
                              <td colSpan={4} className="px-3 py-2 text-white/30 text-center">
        ...
        <div className="flex gap-3 p-5 border-t border-white/10 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2.5 bg-white/5 hover:bg-white/10 text-white/70 rounded-xl text-sm transition-colors"
          >
            ביטול
          </button>
        </div>
